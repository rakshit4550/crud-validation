...
const client = twilio(process.env.TWILIO_STD,process.env.TWILIO_AUTH_TOKEN);
...
export const register = catchAsyncError(async(req,res,next) => {
    try {
        ...
        if(registerationAttempsByUser.length > 3) {
            return next(
...
export const login = catchAsyncError(async(req,res,next) => {
    ...
    if (!email || !password) {
      ...
      if(!user) {
        return next(new ErrorHandler("Invalid email or paassword",400))
      }
...
export const forgotPassword = catchAsyncError(async (req, res, next) => {
    ...
    awaituser.save({validateBeforeSave:false});
...
export const resetassword = catchAsyncError(async (req, res, next) => {
